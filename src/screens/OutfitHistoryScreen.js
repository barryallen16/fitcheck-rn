...
export default function OutfitHistoryScreen({ navigation }) {
  const { outfits, wardrobe, wearOutfit, unwearOutfit, deleteOutfit } = useApp();
  const [tab, setTab] = useState('unworn');
  ...
  return (
    <SafeAreaView style={s.c} edges={['top']}>
      ...
      <View style={s.tabs}>
        <TouchableOpacity style={[s.tab, tab === 'unworn' && s.tabActive]} onPress={() => setTab('unworn')}>
          <Text style={[s.tabTxt, tab === 'unworn' && s.tabTxtActive]}>Unworn ({unworn.length})</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[s.tab, tab === 'worn' && s.tabActive]} onPress={() => setTab('worn')}>
          <Text style={[s.tabTxt, tab === 'worn' && s.tabTxtActive]}>Worn ({worn.length})</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <OutfitCard
            ...
            onWear={(id) => wearOutfit(id, todayStr())}
            onUnwear={unwearOutfit}
            onDelete={(id) => Alert.alert('Delete?', 'Remove this outfit?', [
        ...
        ListEmptyComponent={
          <View style={s.empty}>
            <Ionicons name={tab === 'unworn' ? 'layers-outline' : 'shirt-outline'} size={52} color={COLORS.textMuted} />
            <Text style={s.emptyTitle}>{tab === 'unworn' ? 'No Unworn Outfits' : 'No Worn Outfits'}</Text>
            <Text style={s.emptySub}>{tab === 'unworn' ? 'Generate outfits to see them here' : 'Mark outfits as worn to track them'}</Text>
          </View>
        }
      />
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  emptySub: { color: COLORS.textDim, fontSize: TYPE.sm, textAlign: 'center', marginTop: GAP.sm, paddingHorizontal: GAP.xxxl },
});