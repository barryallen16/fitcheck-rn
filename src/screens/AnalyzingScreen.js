// src/screens/AnalyzingScreen.js
...
export default function AnalyzingScreen({ route, navigation }) {
  ...
  async function run() {
    ...
    for (let i = 0; i < images.length; i++) {
      ...
      try {
        ...
        const g = addGarment({ ...analysis, imageUri: images[i].uri });
  ...
  if (!done) {
    return (
      <SafeAreaView style={s.container} edges={['top']}>
        <View style={s.hdr}>
          <Text style={s.hdrTxt}>Analyzing Garments</Text>
        </View>
        <View style={s.center}>
          ...
          <View style={s.bar}>
            <Animated.View style={[s.fill, { width: pWidth }]} />
          </View>
          <Text style={s.hint}>VLM is inspecting colors, fabric, cut & details</Text>
        </View>
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      <View style={s.hdr}>
        <Text style={s.hdrTxt}>Analysis Complete</Text>
      </View>
        ...
        <View style={s.summary}>
          {okCount > 0 && (
            <View style={s.sumItem}>
              ...
              <Text style={s.sumNum}>{okCount}</Text>
              <Text style={s.sumLbl}>Analyzed</Text>
            </View>
          )}
          {failCount > 0 && (
            <View style={s.sumItem}>
              <Ionicons name="alert-circle" size={26} color={COLORS.red} />
              <Text style={[s.sumNum, { color: COLORS.red }]}>{failCount}</Text>
        ...
        {results.map((r, i) => (
          <View key={i} style={[s.resCard, !r.ok && s.resErr]}>
            ...
            <View style={{ flex: 1, marginHorizontal: GAP.md }}>
              {r.ok ? (
                <>
                  <Text style={s.resTitle}>{r.garment.summary}</Text>
                  <Text style={s.resCat}>{r.garment.category}</Text>
                  <Text style={s.resDesc} numberOfLines={2}>
                    {r.garment.analyzed_garment}
                  </Text>
                </>
              ) : (
                <>
                  <Text style={s.resErrTitle}>Failed</Text>
                  <Text style={s.resErrMsg} numberOfLines={3}>
                    {r.error}
                  </Text>
                </>
              )}
...
const s = StyleSheet.create({
  ...
  resErr: { borderColor: COLORS.redMuted, backgroundColor: 'rgba(232,84,84,0.04)' },
  resImg: { width: 52, height: 52, borderRadius: RAD.sm, backgroundColor: COLORS.cardLight },
  resTitle: { color: COLORS.text, fontSize: TYPE.base, fontWeight: '600' },
  resCat: {
  ...
  resDesc: { color: COLORS.textDim, fontSize: TYPE.xs, marginTop: 2 },
  resErrTitle: { color: COLORS.red, fontSize: TYPE.base, fontWeight: '600' },
  resErrMsg: { color: COLORS.textDim, fontSize: TYPE.xs, marginTop: 2 },
